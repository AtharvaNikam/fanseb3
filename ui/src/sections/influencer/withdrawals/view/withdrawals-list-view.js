...
import { useGetInfluencerWithdraws } from 'src/api/influencerWithdraws';
import UserTableFiltersResult from '../withdrawals-table-filters-result';
import UserTableRow from '../withdrawals-table-row';
...
export default function WithdrawalsListView() {
  ...
  const { influencerWithdraws, refreshInfluencersWithdraws } = useGetInfluencerWithdraws();
    ...
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="List"
          links={[
            { name: 'Dashboard', href: paths.influencer_dashboard.root },
            { name: 'Withdrawals', href: paths.influencer_dashboard.withdrawals.root },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.influencer_dashboard.withdrawals.new}
              variant="contained"
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (withdrawal) =>withdrawal?.influencer.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
