...
const TABLE_HEAD = [
  { id: 'name', label: 'Brand Name', width: 180 },
...
export default function WithdrawalsListView() {
  ...
  const { brandWithdraws, refreshBrandWithdraws } = useGetBrandWithdraws(id);
  ...
  const handleActivateUser = useCallback(
    async (id_r) => {
      const userIndex = tableData.findIndex((row) => row.id === id_r);
      ...
      if (userIndex !== -1) {
        ...
        try {
          await axiosInstance.patch(`/api/users/${id_r}`, inputData);
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Withdrawals', href: paths.brand_dashboard.brands.withdrawals(id).root },
            { name: 'List' },
          ]}
          action={
            <Button
