...
import { useGetRefunds } from 'src/api/refunds';
...
export default function RefundsListView() {
  ...
  const { refunds, refreshRefunds } = useGetRefunds();
  ...
  const handleEditRow = useCallback(
    (id_r) => {
      router.push(paths.admin_dashboard.refunds.edit(id_r));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Refunds', href: paths.admin_dashboard.refunds.root },
            { name: 'List' },
          ]}
          // action={
          //   <Button
