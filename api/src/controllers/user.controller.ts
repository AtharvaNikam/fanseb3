...
export class UserController {
  ...
  async register(
    ...
    try {
      ...
      if (userData.isInfluncer) {
        this.userRepository.influencerBalances(savedUser.id).create(
          {
            influencer_commision_rate: 0,
          },
          {transaction: tx},
        );
      }
  ...
  async findInfluencers(
    ...
    const specificFilter: Filter<User> = {
      ...filter,
      include: [
        {relation: 'brands'},
        {relation: 'reels'},
  ...
  async getSingleInfluencer(
    ...
    const user = await this.userRepository.findOne({
      ...
      include: ['brands', 'reels', 'userProfile'],
    });
    return Promise.resolve({
      ...user,
    });
